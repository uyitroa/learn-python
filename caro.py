...
while True:
	...
	for x in range(len(mustcrossx)):
		cross(mustcrossx[x],mustcrossy[x])
	for x in range(len(mustcercle)):
		cercle(mustcerclex[x],mustcercley[x])
	if clicked == True:
		clicked = False
					...
					for y in range(len(squarey)):
						if y % 2 == 0:
							if ym <= squarey[y + 1] and ym >= squarey[y]:
								...
								if turn == 'first':
									mustcrossx.append(squarex[x])
									mustcrossy.append(squarey[y])
									turn = 'second'
								else:
									mustcerclex.append(squarex[x])
									mustcercley.append(squarey[y])
									turn = 'first'
