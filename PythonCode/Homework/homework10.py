...
def play():
	global now,tm,year,month
	#print(now)
	birt = input("When was he born? ")
	tm = birt.split("-")
	tm.reverse()
	now = input("When was he dead: ")
	now = now.split("-")
	now.reverse()
	year = calculyear()
	#print("Year: ",year)
	...
	while again == 'yes':
		for x in range(2):
			...
			if x == 0:
				...
				second = int(day)
		if first > second:
			print("The first person - second person: ",first - second)
		elif first < second:
			print("The second person - first person: ",second - first)
		else:
			print("==")
